/* Code to save information in genie standard root output to a HEPevt format, specifically for coherent single photon event
 * Initially written by Maya Moore, in summer of 2021
 * Updated by Guanqun Ge
 */
...
void coherentPhoton(const char* pathname) {
	...
	TFile* tfin = new TFile((path+"/gntp.0.flat.root").c_str(), "read");
	TTree* tin = (TTree*)tfin->Get("gst");
